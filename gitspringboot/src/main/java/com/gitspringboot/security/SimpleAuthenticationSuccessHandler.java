package com.gitspringboot.security;
	...
	public void onAuthenticationSuccess(HttpServletRequest arg0, HttpServletResponse arg1, Authentication authentication) throws IOException, ServletException {
			...
			authorities.forEach(authority -> {
				if(authority.getAuthority().equals("ROLE_CLIENT")) {
					try { 
						redirectStrategy.sendRedirect(arg0, arg1, "/certHomeClient");
					} catch (Exception e) {
						
						e.printStackTrace();
					} 
				} else if(authority.getAuthority().equals("ROLE_TECH")) { 
						try { redirectStrategy.sendRedirect(arg0, arg1, "/certHomeTech");
						} catch (Exception e) { 
